...
async function main() {
  ...
  const tx = await token.mint(mintToAddress, ethers.parseUnits(mintAmount, 18));
  await tx.wait();

  console.log(`Minted ${mintAmount} tokens to ${mintToAddress} using token at ${tokenContractAddress}`);
}
