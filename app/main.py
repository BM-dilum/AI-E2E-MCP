...
def create_todo(todo_in: TodoCreate) -> TodoRead:
    todo = store.create(todo_in.title, todo_in.description)
    return TodoRead.model_validate(todo)


@app.get("/todos", response_model=list[TodoRead])
def list_todos() -> list[TodoRead]:
    return [TodoRead.model_validate(todo) for todo in store.list_all()]
