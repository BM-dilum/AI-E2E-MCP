...
contract LoggingContract {
        ...
        returns (SessionDataEntry[] memory, uint256 totalPages)
    {
        ...
        uint256 startIndex = totalSessions - (page * effectivePageSize) - 1;
        uint256 endExclusive = startIndex + 1 > effectivePageSize ? startIndex + 1 - effectivePageSize : 0;

        uint256 resultLength = startIndex - endExclusive + 1;
        SessionDataEntry[] memory results = new SessionDataEntry[](resultLength);

        uint256 resultIndex = 0;
        for (uint256 i = startIndex + 1; i > endExclusive; i--) {
