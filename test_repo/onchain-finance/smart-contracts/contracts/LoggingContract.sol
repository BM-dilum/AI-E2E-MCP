...
contract LoggingContract {
    ...
    function uploadLogs(
        ...
        for (uint256 i = 0; i < logEntries.length; i++) {
            ...
            ToolCall[] memory toolCalls = logEntry.toolCalls;
            ...
            for (uint256 j = 0; j < toolCalls.length; j++) {
                storedLog.toolCalls.push();
                ToolCall storage storedToolCall = storedLog.toolCalls[storedLog.toolCalls.length - 1];
                storedToolCall.name = toolCalls[j].name;
                storedToolCall.args = toolCalls[j].args;
                storedToolCall.result = toolCalls[j].result;
            }
        }
    ...
    function getLogs(
        ...
        for (uint256 i = start; i < end; i++) {
            ...
            results[resultIndex].logs = new LogEntry[](storedEntry.logs.length);

            for (uint256 j = 0; j < storedEntry.logs.length; j++) {
                LogEntry storage storedLog = storedEntry.logs[j];
                results[resultIndex].logs[j].userRequest = storedLog.userRequest;
                results[resultIndex].logs[j].response = storedLog.response;
                results[resultIndex].logs[j].toolCalls = new ToolCall[](storedLog.toolCalls.length);

                for (uint256 k = 0; k < storedLog.toolCalls.length; k++) {
                    ToolCall storage storedToolCall = storedLog.toolCalls[k];
                    results[resultIndex].logs[j].toolCalls[k].name = storedToolCall.name;
                    results[resultIndex].logs[j].toolCalls[k].args = storedToolCall.args;
                    results[resultIndex].logs[j].toolCalls[k].result = storedToolCall.result;
                }
            }
        }
